<?php
	...
	if(isset($_POST['itemDetailsItemNumber'])){
		...
		$metrics = htmlentities($_POST['itemDetailsMetrics']);
		...
		if(!empty($itemNumber) && !empty($itemName) && isset($quantity) && isset($unitPrice)){
			...
			if($stockStatement->rowCount() > 0){
				...
				$insertItemSql = 'INSERT INTO item(itemNumber, itemName, discount, stock, unitPrice,metrics, status, description) VALUES(:itemNumber, :itemName, :discount, :stock, :unitPrice,:metrics, :status, :description)';
				$insertItemStatement = $conn->prepare($insertItemSql);
				$insertItemStatement->execute(['itemNumber' => $itemNumber, 'itemName' => $itemName, 'discount' => $discount, 'stock' => $quantity, 'unitPrice' => $unitPrice,'metrics' => $metrics, 'status' => $status, 'description' => $description]);
