<?php
	...
	if(isset($_POST['itemNumber'])) {
		...
		$itemDetailsMetrics = htmlentities($_POST['itemDetailsMetrics']);
		...
		if(!empty($itemNumber) && !empty($itemName) && isset($itemDetailsQuantity) && isset($itemDetailsUnitPrice)){
			...
			$updateItemDetailsSql = 'UPDATE item SET itemName = :itemName, discount = :discount, stock = :stock, unitPrice = :unitPrice, metrics = :metrics, status = :status, description = :description WHERE itemNumber = :itemNumber';
			$updateItemDetailsStatement = $conn->prepare($updateItemDetailsSql);
			$updateItemDetailsStatement->execute(['itemName' => $itemName, 'discount' => $discount, 'stock' => $newStock, 'unitPrice' => $itemDetailsUnitPrice,'metrics' => $itemDetailsMetrics, 'status' => $status, 'description' => $description, 'itemNumber' => $itemNumber]);
