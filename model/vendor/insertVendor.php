<?php
	...
	if(isset($_POST['vendorDetailsStatus'])){
		...
		if(isset($fullName) && isset($mobile) && isset($address)) {
			...
			$sql = 'INSERT INTO vendor(fullName, email, mobile, address, city, status) VALUES(:fullName, :email, :mobile, :address, :city, :status)';
			$stmt = $conn->prepare($sql);
			$stmt->execute(['fullName' => $fullName, 'email' => $email, 'mobile' => $mobile, 'address' => $address, 'city' => $city, 'status' => $status]);
