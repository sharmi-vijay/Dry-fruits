<?php
	...
	if(isset($_POST['customerDetailsCustomerFullName'])){
		...
		if(isset($fullName) && isset($mobile) && isset($address)) {
			...
			$sql = 'INSERT INTO customer(fullName, email, mobile, address) VALUES(:fullName, :email, :mobile, :address)';
			$stmt = $conn->prepare($sql);
			$stmt->execute(['fullName' => $fullName, 'email' => $email, 'mobile' => $mobile, 'address' => $address]);
